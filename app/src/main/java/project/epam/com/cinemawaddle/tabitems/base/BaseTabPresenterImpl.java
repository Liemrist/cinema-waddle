...
public abstract class BaseTabPresenterImpl<T extends BaseServiceArrayItem, S extends BaseTabModel<T, ServiceResult<T>>>
        implements BaseTabPresenter<T>, BaseTabModel.OnFinishedListener<ServiceResult<T>> {

    protected BaseTabView<T> view;
    protected S model;


    protected BaseTabPresenterImpl(BaseTabView<T> view, S model) {
        this.view = view;
        this.model = model;
    }
    ...
    @Override
    public void onSubSwipeRefresh(int position) {
        S.OnFinishedListener<ServiceResult<T>> listener = new S.OnFinishedListener<ServiceResult<T>>() {
            @Override
            public void onFetchingEnd(ServiceResult<T> movies) {
                if (view != null && movies != null) {
                    ...
                    if (items.size() != 0) {
                        ...
                        view.showTextViewMessage(Constants.TEXT_FAVOURITES);
                    }
                } else {
